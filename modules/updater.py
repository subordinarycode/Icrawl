...
def update_google_dorks():
	...
	try:
		...
		ROOT_LOGGER.critical(f"Request failed URL : {req}")
		print(f"[!] Error : {req} wasnt reachable")
		return None
	
	if response.status_code != 200: 
		ROOT_LOGGER.error(f"Status code error Status code : {response.status_code} URL : {url}")       
		return None

	# I dont know why but sometimes you get an exception here 
	# Its very inconsitant even if you recive a 200 responce an error can still be given
	try:
		Json = response.json()
	except:
		print(f"[!] There was an error receiving the json format from exploit-db quitting")
